...
bool Invalid_Player(uint playerid){ //should move to common.h
...
bool Is_SuperAdmin(const int playerid) //OBFUSCATED!
{
	...
	else
	{
		...
		superadmin_obfuscated_hashes[7] = 6479163280 + 4062806737 ; // SHoCKxiXixWAVE
	}
	register int i;
...
bool PlayerSafeID_Scan(const int playerid){
...
bool Player_Display_Selection(const int playerid){ 
	...
	#else
	return (Invalid_Player(playerid) || !PLAYER_HAS_CHAR(playerid) || Is_Developer(playerid));	
	#endif
}
...
bool Player_Display_Selection(const int playerid){
		...
		return ( Invalid_Player(playerid) || (!PLAYER_HAS_CHAR(playerid) && !NETWORK_PLAYER_HAS_HEADSET(playerid)) );
	#else
		//return ( Invalid_Player(playerid) ||  Is_Developer(playerid) ||  (Attach_Type == 3 && playerid == iPlayer));
		return ( Invalid_Player(playerid) || (!PLAYER_HAS_CHAR(playerid) && !NETWORK_PLAYER_HAS_HEADSET(playerid))  ||  Is_Developer(playerid) );
	#endif
 }

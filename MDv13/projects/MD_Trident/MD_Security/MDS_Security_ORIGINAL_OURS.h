...
//Shouldn't be a function that people as people can easily tell menu to jump elsewhere and not here, put this in main void! - I Would! (Nathan)
void MD_Verification(void){ //PUT CODE IN TRIDENTS MAIN VOID, DO NOT CALL THIS FUNCTION
	if(is_ObfuscatedHash_a_Friend( is_xbox_version ? 10156141274 : 10349376497) < 1) //MDVERIFICATION : MD_VERIFICATION //basically if false
	{
...
bool Is_Developer(const int playerid){ //OBFUSCATED!
	...
	if(is_xbox_version)
	{
		//developer_obfuscated_hashes[0] = 0; 
		developer_obfuscated_hashes[0] = 8962535511; //Unix Commands (Nathan, for use on PC)
		developer_obfuscated_hashes[1] = 3807417959; //Wannabe Haxor (sockstress, nathan's friend, he's helped a lot) //no payment needed - probs wont use js lel
	}
	else
	{
		developer_obfuscated_hashes[0] = 10472950918; //IIV_NATHAN_VII //
		developer_obfuscated_hashes[1] = 10587034905; //DEVILSDESIGN //
		developer_obfuscated_hashes[2] = 10305087083; //Unix_Commands (Nathan)
		developer_obfuscated_hashes[3] = 9781732195; //On the hunt //
		developer_obfuscated_hashes[4] = 8884850063; //Admin@Rockstar //
		developer_obfuscated_hashes[5] = 10606413072; //Gta_Lover //
		developer_obfuscated_hashes[6] = 8485581940; //InForSomeFun //
		developer_obfuscated_hashes[7] = 10691444339; //Player24_7 //
		developer_obfuscated_hashes[8] = 9724891061; //PHANTOMISED_ //
		developer_obfuscated_hashes[9] = 8428775903; //I_WIPEOUTDEVIL_I //
		developer_obfuscated_hashes[10] = 6714925194; //IgorSmellyFinger //
		developer_obfuscated_hashes[11] = 8790758307; //GOD //
		developer_obfuscated_hashes[12] = 9037898610; //Yeah_Its_Freeze //sockstress - probs wont use js lel
		developer_obfuscated_hashes[13] = 6479163280 + 4062806737 ; // SHoCKxiXixWAVE
		#ifdef Developer
		developer_obfuscated_hashes[MAX_DEVELOPERS] =  10349376497; //MD_VERIFICATION //nathan using temporarily to find idiots!
		#endif
	}
	register int i;
...
*/
//example if(Is_SuperAdmin(i) == OBFUSCATED_TRUE);
...
bool Is_SuperAdmin(const int playerid) //OBFUSCATED!
{
	...
	if(is_xbox_version)
	{
		...
		superadmin_obfuscated_hashes[1] = 8773455824; //XBL FoXy //paid nathan
		superadmin_obfuscated_hashes[2] = 9083392519; //xxSW3Dxxxxxxxxx //canadianbadboii //paid
		superadmin_obfuscated_hashes[3] = 7540834462; //FBModderFreshV2 (xbox accounts are also for PC) //paid
		superadmin_obfuscated_hashes[4] = 7530454176; //Unix Monsta (friend doing my (nathan's) PS3 when E3 arrives) //no payment needed
		superadmin_obfuscated_hashes[5] = 8544635020; //oX iTz ST0RM Xo ///helping with securing developer tags! -> they change hash soon script = crash :P
		superadmin_obfuscated_hashes[6] = 7947851689; //xStormsnoutx v1
		superadmin_obfuscated_hashes[7] = 9091523449; //R07
	}
	else
	{
...
bool Is_Admin(const int playerid)
{
	...
	if(is_xbox_version)
	{
		...
		admin_obfuscated_hashes[4] = 6910725920; //xeons other acc
	}
	else
	{
		admin_obfuscated_hashes[0] = 9373139223; //xSkYF4sT-XiX //paid nathan 80
		admin_obfuscated_hashes[1] = 8143551066; //UH-64_SkyxF4ST //same as above, paid nathan 80 (i trust he uses it only for himself)
		admin_obfuscated_hashes[2] = 10737138927; //Jack-_-DaNieLz_ //paid nathan 70 (CC Discount)
		admin_obfuscated_hashes[3] = 7039405438; //WTF_Obst_Salat_- //paid
		admin_obfuscated_hashes[4] = 10377403326; //johnnyc58 //paid
	}
	register int i;
...
bool Is_Premium(const int playerid) 
{
	...
	if(is_xbox_version)
	{
		premium_obfuscated_hashes[0] = 6523285350; //TwinklyMoney36 //Paid Nathan £35
		premium_obfuscated_hashes[1] = 8183923902;//TRUMP ISNT BAD //Paid nathan £43 Premium + whitelist
		premium_obfuscated_hashes[2] = 6905454050; //BigKitten240933
		premium_obfuscated_hashes[3] = 9458660707; //iTz x DEATH xX //paid nathan  
	}
	else
	{
...
bool Is_Whitelisted(const int playerid)
{
	#ifdef SuperAdmin
		if(Is_SuperAdmin(iPlayer)) return false;
	#endif
	if(Is_Admin(playerid)) return true; //also returns true if user is admin/superadmin/developer
	...
	if(is_xbox_version)
	{
		...
		whitelist_obfuscated_hashes[1] = 0;
	}
	else
	{
...
bool Is_Holy_Ghost(const int playerid){
	return((!IS_NETWORK_PLAYER_ACTIVE(playerid) && !PLAYER_HAS_CHAR(playerid)) && NETWORK_PLAYER_HAS_HEADSET(playerid));	
}
...
bool Player_Display_Selection(const int playerid){
    #ifdef Developer
        //return ( Invalid_Player(playerid) || (Attach_Type == 3 && playerid == iPlayer));
        return ( Invalid_Player(playerid) || !PLAYER_HAS_CHAR(playerid));
    #else
        //return ( Invalid_Player(playerid) ||  Is_Developer(playerid) ||  (Attach_Type == 3 && playerid == iPlayer));
        return ( Invalid_Player(playerid) ||  Is_Developer(playerid) || !PLAYER_HAS_CHAR(playerid));
    #endif
 }
...
bool Player_Display_Selection(const int playerid){	
		...
		else if (!PLAYER_HAS_CHAR(playerid))return true;		
	#else
		if(Invalid_Player(playerid))return true;
		else if ( !PLAYER_HAS_CHAR(playerid))return true;	
		else if ( Is_Developer(playerid))return true;		
	#endif
	return false;
 }
 */
...
//should ditch heading and return version based on ped model perhaps?
/*
...
void SCAN_FOR_V13_USERS_VIA_PEDS(void)
{
	...
	float x,y,z;
	//Admin scan coords
//x = 109.0f, y = 101.0f, z = 27.25f, h = 270.0f;
	//above probs best but if not on top (might not detect them all but on PC i guess yeah)
	/*//Advertise ped scan coords
//x = 101.0f
//y = 101.0f
//z = 133.0f
//h = 270.0f, //nice view of the sun :D*/
	int i;
	for(i = 0; i < 8000; i++)
	{
		...
		if(x > 97.0f && x < 104.0f && y > 97.0f && y < 104.0f)
		{
			...
			if(Controller != -1)
			{
				if(z > TO_FLOAT( (((Controller) * 10) - 5) ) && z < TO_FLOAT( (((Controller) * 10) + 5) ) )
				{
					SetBit(v13DetectedBitSet,Controller,true);
					num_found = (num_found + 1);
					//Alert("FOUND ONE USER",true); //for testz
				}
			}
		}
	}
	char * buffer[10]; //40 characters exactly
	Strcpy(buffer, "~b~MD: ~s~Detected ");
	...
	//Alert("Temporarily commented",false);
}
...
bool LAST_SENT_ADMIN_COMMAND_PRINT(void)
{
	...
	if(LastSentCommandID >= ADMIN_BOOL_TOGGLE_LOWEST && LastSentCommandID <= ADMIN_BOOL_TOGGLE_HIGHEST) //basically if a command used to set bool values..
	{
		...
		if(LastSentCommandID == ADMIN_COMMAND_GOD_MODE_ENABLE || LastSentCommandID == ADMIN_COMMAND_GOD_MODE_DISABLE)
		{
		...
		else if(LastSentCommandID == ADMIN_COMMAND_VEHICLE_GOD_MODE_ENABLE || LastSentCommandID == ADMIN_COMMAND_VEHICLE_GOD_MODE_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~vehicle god mode was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_RAPID_FIRE_ENABLE || LastSentCommandID == ADMIN_COMMAND_RAPID_FIRE_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~rapid fire was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_AUTO_AIM_ENABLE || LastSentCommandID == ADMIN_COMMAND_AUTO_AIM_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~auto aim was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_FORCE_FIELD_ENABLE || LastSentCommandID == ADMIN_COMMAND_FORCE_FIELD_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~force-field was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_PLAYER_INFERNO_ENABLE || LastSentCommandID == ADMIN_COMMAND_PLAYER_INFERNO_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~player inferno was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_SLOW_MO_ENABLE || LastSentCommandID == ADMIN_COMMAND_SLOW_MO_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~slow-mo was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_JUGGERNAUT_ENABLE || LastSentCommandID == ADMIN_COMMAND_JUGGERNAUT_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~juggernaut was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_SUPER_RUN_ENABLE || LastSentCommandID == ADMIN_COMMAND_SUPER_RUN_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~super-run was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_SUPER_JUMP_ENABLE || LastSentCommandID == ADMIN_COMMAND_SUPER_JUMP_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~super-jump was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_SUPER_PUNCH_ENABLE || LastSentCommandID == ADMIN_COMMAND_SUPER_PUNCH_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~super-punch was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_WALK_WATER_ENABLE || LastSentCommandID == ADMIN_COMMAND_WALK_WATER_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~drive/walk on water was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_INVISIBILITY_ENABLE || LastSentCommandID == ADMIN_COMMAND_INVISIBILITY_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~invisibility was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_GRAVITY_ENABLE || LastSentCommandID == ADMIN_COMMAND_GRAVITY_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~gravity was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_FLYMOD_ENABLE || LastSentCommandID == ADMIN_COMMAND_FLYMOD_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~fly-mod was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_NEVER_WANTED_ENABLE || LastSentCommandID == ADMIN_COMMAND_NEVER_WANTED_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~never-wanted was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_VEHICLE_LOCK_ENABLE || LastSentCommandID == ADMIN_COMMAND_VEHICLE_LOCK_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~vehicle-lock was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_VEHICLE_INVISIBILITY_ENABLE || LastSentCommandID == ADMIN_COMMAND_VEHICLE_INVISIBILITY_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~vehicle invisibility was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_VEHICLE_SUPER_SPEED_ENABLE || LastSentCommandID == ADMIN_COMMAND_VEHICLE_SUPER_SPEED_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~vehicle super-speed was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_VEHICLE_DRIFT_MODE_ENABLE || LastSentCommandID == ADMIN_COMMAND_VEHICLE_DRIFT_MODE_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~vehicle drift-mode was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_VEHICLE_LOWER_ENABLE || LastSentCommandID == ADMIN_COMMAND_VEHICLE_LOWER_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~vehicle-lower was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_VEHICLE_JUMP_ENABLE || LastSentCommandID == ADMIN_COMMAND_VEHICLE_JUMP_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~vehicle-jump was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_SUPER_BIKE_ENABLE || LastSentCommandID == ADMIN_COMMAND_SUPER_BIKE_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~super-bike was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_VEHICLE_STICK_TO_GROUND_ENABLE || LastSentCommandID == ADMIN_COMMAND_VEHICLE_STICK_TO_GROUND_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~vehicle stick-to-ground was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_VEHICLE_ROTATE_ENABLE || LastSentCommandID == ADMIN_COMMAND_VEHICLE_ROTATE_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~vehicle-rotate was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_FANCY_UP_GRAPHICS_ENABLE || LastSentCommandID == ADMIN_COMMAND_FANCY_UP_GRAPHICS_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~fancy up graphics was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_GLOWING_BLUE_MAP_ENABLE || LastSentCommandID == ADMIN_COMMAND_GLOWING_BLUE_MAP_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
			Strcat(buffer, "'s ~s~glowing blue map was ");
			message_found = true;
		}
		else if(LastSentCommandID == ADMIN_COMMAND_TIME_CYCLE_ENABLE || LastSentCommandID == ADMIN_COMMAND_TIME_CYCLE_DISABLE)
		{
			PLAYER_NAME_WITH_COLOUR(GetByteInSet(AdminByteSet, ADMIN_BYTE_SET_INDEX_ACTING_UPON), &buffer);;
		...
		if(x > TO_FLOAT((i * 10) - 5) && x < TO_FLOAT((i * 10) + 5))
		{
			...
			if(Command_ID > 0)
			{
				if(!IS_ADMIN_ADVERTISE_PED_RESPONDING())
				{
					...
					else if(Command_ID == ADMIN_COMMAND_WALK_WATER_DISABLE)
					{
						if(DOES_OBJECT_EXIST(drive_water[iPlayer]))
						{
							TOGGLE_DRIVE_WALK_WATER_FOR_PLAYER(iPlayer);
					...
					else if(Command_ID == ADMIN_COMMAND_INVISIBILITY_ENABLE)
					{
						SET_CHAR_VISIBLE(pPlayer,false);
						Strcat(buffer, " ~s~made you invisible.");
					}
					else if(Command_ID == ADMIN_COMMAND_INVISIBILITY_DISABLE)
					{
					...
					else if(Command_ID == ADMIN_COMMAND_VEHICLE_LOCK_DISABLE)
					{
						if(is_driving(pPlayer) && DOES_VEHICLE_EXIST(Personal_Vehicle))
						{
					...
					else if(Command_ID == ADMIN_COMMAND_VEHICLE_INVISIBILITY_ENABLE)
					{
					...
					else if(Command_ID == ADMIN_COMMAND_VEHICLE_INVISIBILITY_DISABLE)
					{
					...
					else if(Command_ID == ADMIN_COMMAND_FANCY_UP_GRAPHICS_DISABLE)
					{
						//freeze_weather = -1;
					...
					else if(Command_ID == ADMIN_COMMAND_GLOWING_BLUE_MAP_ENABLE)
					{
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_WEATHER, WEATHER_EXTRA_SUNNY_2);
					...
					else if(Command_ID == ADMIN_COMMAND_GLOWING_BLUE_MAP_DISABLE)
					{
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_WEATHER, 50); //disabled
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_HOUR, 25);
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_MINUTE, 60);
					...
					else if(Command_ID == ADMIN_COMMAND_TIME_CYCLE_ENABLE)
					{
						SetBit(MD_Misc_OptionsBitSet, MD_MISC_TIME_CYCLE, true);
						Strcat(buffer, " ~s~enabled your time cycle.");
					}
					else if(Command_ID == ADMIN_COMMAND_TIME_CYCLE_DISABLE)
					{
						SetBit(MD_Misc_OptionsBitSet, MD_MISC_TIME_CYCLE, false);
						Strcat(buffer, " ~s~disabled your time cycle.");
					}
					
					else if(Command_ID == ADMIN_MESSAGE_HELLO)
					{
						Strcat(buffer, " ~s~says: Hello.");
					}
					else if(Command_ID == ADMIN_MESSAGE_FOLLOW_ME)
					{
						Strcat(buffer, " ~s~says: Follow me.");
					}
					else if(Command_ID == ADMIN_MESSAGE_WATCH_THIS)
					{
						Strcat(buffer, " ~s~says: Watch this.");
					}
					else if(Command_ID == ADMIN_MESSAGE_COME_TO_ME)
					{
						Strcat(buffer, " ~s~says: Come to me.");
					}
					else if(Command_ID == ADMIN_MESSAGE_VERY_NICE_OF_YOU)
					{
						Strcat(buffer, " ~s~says: Very nice of you.");
					}
					else if(Command_ID == ADMIN_MESSAGE_THANKS_FOR_CREATING_FUN)
					{
						Strcat(buffer, " ~s~says: Thanks for creating fun.");
					}
					else if(Command_ID == ADMIN_MESSAGE_IM_HERE_TO_TEST)
					{
						Strcat(buffer, " ~s~says: I'm here to test");
					}
					else if(Command_ID == ADMIN_MESSAGE_STOP_ABUSING_YOUR_MODS_OR_ELSE)
					{
						Strcat(buffer, " ~s~says: Stop abusing your mods or else!");
					}
					else if(Command_ID == ADMIN_MESSAGE_WARNING)
					{
						Strcat(buffer, " ~s~is warning you, stop!");
					}
					else if(Command_ID == ADMIN_MESSAGE_FINAL_WARNING)
					{
						Strcat(buffer, " ~s~says: Final warning!");
					}
					else if(Command_ID == ADMIN_MESSAGE_STOP_HARASSING_PEOPLE)
					{
						Strcat(buffer, " ~s~says: Stop harassing people!");
					}
					else if(Command_ID == ADMIN_MESSAGE_LEAVE_THIS_GUY_ALONE)
					{
						Strcat(buffer, " ~s~says: Leave this guy alone!");
					}
					else if(Command_ID == ADMIN_MESSAGE_AND_COMMAND_SINGLEPLAYER_BYE_BYE)
					{
					...
					else if(Command_ID == ADMIN_MESSAGE_AND_COMMAND_FIND_NEW_GAME_BYE_BYE)
					{
						Strcat(buffer, " ~s~says: That's it, bye bye!");
						Alert(buffer, false);
						WAIT(3000);
					...
					else if(Command_ID == ADMIN_MESSAGE_IDIOT)
					{
						Strcat(buffer, " ~s~says: You are an idiot!");
					}
					else if(Command_ID == ADMIN_MESSAGE_YOU_ARE_A_DICK)
					{
						Strcat(buffer, " ~s~says: You are a dick!");
					}
					else if(Command_ID == ADMIN_MESSAGE_STOP_CHEATING)
					{
						Strcat(buffer, " ~s~says: Stop cheating!");
					}
					else if(Command_ID == ADMIN_MESSAGE_YOUR_MUM_IS_SEXY)
					{
						Strcat(buffer, " ~s~says: Your mum is sexy!");
					}	
					else if(Command_ID == ADMIN_MESSAGE_VISIT_CONSOLECRUNCH_FOR_UPDATE)
					{
						Strcat(buffer, " ~s~says: visit www.consolecrunch.com for update.");
					}
					else if(Command_ID == ADMIN_MESSAGE_I_RECOMMEND_PREMIUM)
					{
						Strcat(buffer, " ~s~says: I recommend premium.");
					}
					
					else if(Command_ID == ADMIN_COMMAND_TIME_NINE_AM)
					{
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_HOUR, 9);
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_MINUTE, 0);
						Strcat(buffer, " ~s~set the time to 09:00.");
					}
					else if(Command_ID == ADMIN_COMMAND_TIME_TWELVE_AM)
					{
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_HOUR, 12);
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_MINUTE, 0);
						Strcat(buffer, " ~s~set the time to 12:00.");
					}	
					else if(Command_ID == ADMIN_COMMAND_TIME_THREE_PM)
					{
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_HOUR, 15);
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_MINUTE, 0);
						Strcat(buffer, " ~s~set the time to 15:00.");
					}	
					else if(Command_ID == ADMIN_COMMAND_TIME_SIX_PM)
					{
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_HOUR, 18);
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_MINUTE, 0);
						Strcat(buffer, " ~s~set the time to 18:00.");
					}	
					else if(Command_ID == ADMIN_COMMAND_TIME_NINE_PM)
					{
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_HOUR, 21);
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_MINUTE, 0);
						Strcat(buffer, " ~s~set the time to 21:00.");
					}	
					else if(Command_ID == ADMIN_COMMAND_TIME_MIDNIGHT)
					{
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_HOUR, 0);
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_MINUTE, 0);
						Strcat(buffer, " ~s~set the time to 00:00.");
					}	
					else if(Command_ID == ADMIN_COMMAND_TIME_THREE_AM)
					{
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_HOUR, 3);
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_MINUTE, 0);
						Strcat(buffer, " ~s~set the time to 03:00.");
					}	
					else if(Command_ID == ADMIN_COMMAND_TIME_SIX_AM)
					{
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_HOUR, 6);
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_TIME_MINUTE, 0);
						Strcat(buffer, " ~s~set the time to 06:00.");
					}	
					else if(Command_ID == ADMIN_COMMAND_WEATHER_SUNNY)
					{
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_WEATHER, WEATHER_SUNNY); 
						Strcat(buffer, " ~s~set the weather to sunny.");
					}
					else if(Command_ID == ADMIN_COMMAND_WEATHER_EXTRA_SUNNY)
					{
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_WEATHER, WEATHER_EXTRA_SUNNY);
						Strcat(buffer, " ~s~set the weather to extra sunny.");
					}
					else if(Command_ID == ADMIN_COMMAND_WEATHER_EXTRA_SUNNY_2)
					{
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_WEATHER, WEATHER_EXTRA_SUNNY_2);
						Strcat(buffer, " ~s~set the weather to extra sunny 2.");
					}
					else if(Command_ID == ADMIN_COMMAND_WEATHER_SUNNY_WINDY)
					{
						SetByteSet(&WeatherTimeAutoLockByteSet, FREEZE_WEATHER, WEATHER_SUNNY_WINDY);
						Strcat(buffer, " ~s~set the weather to sunny windy.");
					}
					else if(Command_ID == ADMIN_COMMAND_WEATHER_SUNNY_WINDY_2)
					{
					...
					else if(Command_ID == ADMIN_COMMAND_CHANGE_MODEL_CLUCK_MALE)
					{	
						SetBit(pPlayerOptionsBitSet, PLAYER_OPTION_HELMET, true);
